...
export function render_afm_like(atoms, opts = {}) {
    ...
    const angstroms_per_pixel = Number.isFinite(opts.angstroms_per_pixel) ? opts.angstroms_per_pixel : 0.1;
    const blur_sigma = Number.isFinite(opts.blur_sigma) ? opts.blur_sigma : 2.0;
    const background_gray = Number.isFinite(opts.background_gray) ? opts.background_gray : 127;
    const invert = !!opts.invert;
    const noise_stddev = Number.isFinite(opts.noise_stddev) ? opts.noise_stddev : 0.0;
    ...
    const bond_wave_width_px = Number.isFinite(opts.bond_wave_width_px) ? opts.bond_wave_width_px : 6.0;
    const bond_wave_amplitude = Number.isFinite(opts.bond_wave_amplitude) ? opts.bond_wave_amplitude : 0.4;
    ...
    const scale_bar_margin_px = Number.isFinite(opts.scale_bar_margin_px) ? opts.scale_bar_margin_px : 12;
    ...
    for (let i = 0; i < atoms.length; i++) {
        ...
        const amp = Math.max(0.05, Math.min(3.0, rA)); // size-driven; later normalized by hmax
    ...
    if (H >= 3 && W >= 3) {
        for (let y = 1; y < H - 1; y++) {
            ...
            for (let x = 1; x < W - 1; x++) {
                const p = row + x;
                const c = height[p];
                const lap = 4.0 * c - height[p - 1] - height[p + 1] - height[p - W] - height[p + W];
